...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const prompt = `אתה אנליסט תחרותי מומחה בשיווק דיגיטלי בישראל. נתח את הנוף התחרותי עבור העסק הבא:

שם עסק: ${businessName}
תחום: ${industry || 'לא צוין'}
${website ? `אתר: ${website}` : ''}
${clientServices?.length ? `שירותים שאנחנו מספקים: ${clientServices.join(', ')}` : ''}
${additionalContext ? `הקשר נוסף: ${additionalContext}` : ''}

בצע ניתוח תחרותי מקיף וענה בפורמט JSON הבא בלבד (ללא markdown, ללא backticks):
{
  "summary": "סיכום קצר של הנוף התחרותי (2-3 משפטים)",
  "competitors": [
    {
      "name": "שם המתחרה",
...
חשוב:
- זהה 3-5 מתחרים רלוונטיים בשוק הישראלי
- התמקד בתחום הספציפי של העסק
- תן המלצות אקטיביות ומעשיות
